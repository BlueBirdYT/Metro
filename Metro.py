...
async def userinfo(ctx, user: discord.Member=None):
    if user is None:
      await client.say('Please tag a user to get user information. Example- ``Muserinfo @user``')
    if ctx.message.author.bot:
      return
    if ctx.message.author.server_permissions.kick_members == False:
      await client.say('**You do not have permission to use this command**')
...
async def warn(ctx, userName: discord.User=None,*, message:str=None):
    if userName is None:
      await client.say('Please tag a person to warn user. Example- ``Mwarn @user <reason>``')
      return
    else:
...
async def announce(ctx, channel: discord.Channel=None, *, msg: str=None):
    ...
    if channel is None or msg is None:
        await client.say('Invalid args. Use this command like ``Mannounce #channel text here``')
        return
    else:
...
client.run('NTM3NjQ1NDQwMDkxMjkxNjY5.DyogUw.m3GBET5ERoiz9Jr-OHhbmj6duZQ')
